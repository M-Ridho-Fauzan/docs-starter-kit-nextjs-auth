...
description: TOTP two-factor authentication for Next.js 16 with Better Auth. Enable 2FA, configure TOTP, manage backup codes, and handle the verification flow.
---
...
2. If the user has 2FA enabled, `auth.api.signInEmail()` returns `{ twoFactorRedirect: true, twoFactorMethods: ["totp"] }` instead of a session token.
...
- [User Components](../ui/user-components.md) — 2FA setup forms
